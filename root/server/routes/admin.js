...
*/
router.get('/Users', getUserCount, async(req,res,next)=>{  
...
*/
router.get('/Users/:id', doesUserExist, apiErrorHandler, async(req, res, next)=>{
    const id = req.params.id;
...
// (GET)/Admin//Users?... - Handle pagination, sorting and filtering of the list of users, using query parameters
// router.get('/User/:id', validateToken, doesUserExist, apiErrorHandler, async(req, res, next)=>{
// //    router.get('/Users?sort=[field, order]&range=[page, perPage]&filter={"title":"bar"}', doesUserExist, apiErrorHandler, async(req, res, next)=>{  
...
//------------------------UNUSED ROUTES DUE TO USING REACT-ADMIN:-------------------------
/**
